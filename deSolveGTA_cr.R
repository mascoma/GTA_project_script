...
outputdir <- "/isi/olga/xin/GTA_project/output/20160701/X_ratio_c_r.txt"
  ...
  if (b == 50){
    ...
    if (b == 50){
      ...
      for (a in 1:50){
        ...
        deltaX <- abs(out[length(times), 2]-out[(length(times)-a), 2]) 
        if (deltaX < 0.00001 && (out[length(times), 2] + out[length(times), 3]) > 1000) {
          b = b + 1
        }
      }
      if (b == 50){
