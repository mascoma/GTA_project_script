...
outputdir <- paste("/isi/olga/xin/GTA_project/output/20160816/GTArecheckmodels_", 
                   k,"_", j, ".txt", sep = "")

range.all <- range((1200*(k-1)+1), (1200*k))
range.sub <- range(range.all[1]+20*(j-1),  ((range.all[1]+20*j)-1))
...
array <- matrix(NA, nrow = 20, ncol = 9)
