...
Gillespie.SSA <- function (lpfreq, time, tau,verbose, method) {
  parms <- c(r=0.1, K=10^9, c=0.001, lp=lpfreq, g=0.1, i=0.001,r=0.001, n=200)
  x0 <- c(X1=9*10^6, X2=8.91*10^8)
  a  <- c("r*(1-(X1+X2)/K)*X1*(1-c)", "c*X1","n*c*lp*g*i*r*X1*X2","r*(1-(X1+X2)/K)*X2")
...
for (i in 1: length(lpfreqs)){
  ...
  temp <- Gillespie.SSA(lpfreqs[i], 20000, 0.05, verbose=T, method="ETL")
  output[[i]]<-temp
